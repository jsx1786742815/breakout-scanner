...
def check_pullback(cand, date):
    ...
    return {"code": code, "name": name, "chg": round(cand["chg"], 2), "close": closes[i],
            ...
            "target": round(prev_high, 2), "main_wan": cand["main_wan"], "industry": cand["industry"]}

def main():
    date = last_trade_date()
    print(f"低吸战法扫描(缩量回踩) | 日期 {date} | 强势股近20日≥15% + 回踩MA5/10 + 缩量<0.7")
    cands = snapshot()
    print(f"初筛候选(当日-3%~+3%): {len(cands)} 只，精算低吸条件...")
    result = []
    ...
    for r in result:
        print(f"  {r['code']} {r['name']} 收{r['close']} 乖离MA5 {r['b5']}% 量比{r['vol_ratio']} 20日{r['chg20']}% "
              f"挂{r['buy_ref']}(MA5)/{r['buy_ref2']}(MA10) 止损{r['stop']}(MA20) 目标{r['target']} {r['industry']}")
